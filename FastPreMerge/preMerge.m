...
meaningMap('203') = 'blank space key';
...
for i=1:length(rawTable.id(:))
    ...
    % linecheck：该行检查的方式，在本实验中：0-不检查，1-作为刺激检查，2-作为反应检查
...
for i=1:length(preGroupedTable.id(:))
    ...
    if i==length(preGroupedTable.id(:)) && ~groupCompleted
        % 如果到了最后，但是有未完成的组
        % 说明后面少了些东西
        % 在本实验中，就是少了一次反应
        lineresp       = '0';
